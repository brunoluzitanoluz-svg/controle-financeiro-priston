...
import { TrendingUp, TrendingDown, Clock, AlertCircle, Users, Bell, Download } from 'lucide-react'
...
export default function Home() {
  ...
  const [dadosExport, setDadosExport] = useState<any>({ receitas: [], despesas: [], contas_pagar: [], contas_receber: [], salarios: [] })
  ...
  useEffect(() => {
    async function carregar() {
      ...
      const { data: cr } = await supabase.from('contas_receber').select('*').eq('status', 'pendente').gte('vencimento', inicio).lte('vencimento', fim)
      if (cr) { setAReceber(cr.reduce((acc, x) => acc + x.valor, 0)); setDadosExport((p: any) => ({ ...p, contas_receber: cr })) }
  ...
  function exportarPDF() {
    ...
    if (dadosExport.salarios.length > 0) {
      ...
      autoTable(doc, {
        startY: y + 4,
        head: [['Socio', 'Valor', 'Status']],
        body: dadosExport.salarios.map((x: any) => [x.socio, `R$ ${x.valor.toFixed(2)}`, x.status]),
        styles: { fontSize: 10 },
