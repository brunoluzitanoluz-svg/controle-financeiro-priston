...
export default function Salarios() {
  ...
  async function adicionar() {
    if (!valor || !mes) return
    const { error } = await supabase.from('salarios_socios').insert({ socio, valor: parseFloat(valor), mes, status, tipo })
    ...
    <main className="min-h-screen bg-[#0f1117] p-8">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="bg-[#1a1d2e] border border-[#2a2d3e] rounded-2xl p-6 mb-6">
          <h2 className="text-white font-semibold mb-4">Novo Lancamento</h2>
          <div className="grid grid-cols-2 gap-3">
            ...
            <select value={tipo} onChange={e => setTipo(e.target.value)} className="bg-[#0f1117] border border-[#2a2d3e] rounded-xl px-4 py-3 text-white focus:outline-none focus:border-indigo-500">
        ...
        <div className="space-y-3">
          {SOCIOS.map(s => {
            ...
            const totalSocio = itens.reduce((acc, x) => acc + x.valor, 0)
            return (
              <div key={s}>
