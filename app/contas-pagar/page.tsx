      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          <div className="bg-[#1a1d2e] border border-[#2a2d3e] rounded-2xl p-4">
            ...
            <p className="text-amber-400 text-xl font-bold">{fmt(totalPendente)}</p>
          ...
          <div className="bg-[#1a1d2e] border border-yellow-400/20 rounded-2xl p-4">
            ...
            <p className="text-yellow-400 text-xl font-bold">{fmt(totalVencendo30)}</p>
          ...
          <div className="bg-[#1a1d2e] border border-emerald-400/20 rounded-2xl p-4">
            ...
            <p className="text-emerald-400 text-xl font-bold">{fmt(totalPago)}</p>
        ...
        <div className="flex gap-2 mb-4">
          <button onClick={() => setAbaAtiva('pendente')} className={`px-5 py-2 rounded-xl text-sm font-semibold transition-colors ${abaAtiva === 'pendente' ? 'bg-amber-500/20 text-amber-400 border border-amber-500/30' : 'bg-[#1a1d2e] text-gray-400 border border-[#2a2d3e] hover:text-white'}`}>
            Pendentes ({pendentes.length})
          </button>
          <button onClick={() => setAbaAtiva('pago')} className={`px-5 py-2 rounded-xl text-sm font-semibold transition-colors ${abaAtiva === 'pago' ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'bg-[#1a1d2e] text-gray-400 border border-[#2a2d3e] hover:text-white'}`}>
            Pagos ({pagos.length})
          </button>
        </div>
        ...
        {listaAtiva.length > 0 && (
          <div className="mt-4 bg-[#1a1d2e] border border-[#2a2d3e] rounded-2xl px-6 py-4 flex justify-between items-center">
            ...
            <span className={`font-bold text-lg ${abaAtiva === 'pendente' ? 'text-amber-400' : 'text-emerald-400'}`}>
              {fmt(abaAtiva === 'pendente' ? totalPendente : totalPago)}
            </span>
          </div>
        )}
